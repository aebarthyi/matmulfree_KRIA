...
################################################################
# This is a generated script based on design: t_matmul
#
...
\
  ...
  # MM2S width must match CoreTop's per-port s_axis width: xDim*aWidth when
  # that fits one HP port (64 for k26_bench, 128 for k26_bench16), else 128
  # per port with the stream split across NUM_DMA parallel DMAs/HP ports
  ...
  if { $num_dma < 1 || $num_dma > 4 } {
    catch {common::send_gid_msg -ssname BD::TCL -id 2099 -severity "ERROR" "NUM_DMA=$num_dma out of range 1..4"}
    return 4
  }

  # PL fabric clock (pl_clk0 = PL0_REF). The static config above is 100 MHz;
  # build_all.sh exports PL_CLK_MHZ to raise it (250 for k26_mmfree370m:
  # CoreTop closes 250 MHz OOC with WNS +0.79 ns, and 4 HP ports x 16 B at
  # 250 MHz lift the weight-stream peak 6.4 -> 16 GB/s ~= the DDR4 wall).
  # The PS config wizard recomputes ACT_FREQMHZ and the derived timing
  # constraints, and createdts picks the new rate up in the clocking fragment.
  set pl_clk_mhz 100
  if { [info exists ::env(PL_CLK_MHZ)] } { set pl_clk_mhz $::env(PL_CLK_MHZ) }
  if { $pl_clk_mhz != 100 } {
    set_property CONFIG.PSU__CRL_APB__PL0_REF_CTRL__FREQMHZ $pl_clk_mhz $zynq_ultra_ps_e_0
  }

  # Extra HP ports for DMAs 1..N-1: enable S_AXI_GP{2+i} at 128 bits and flip
  # the S_AXI_HP{i}_FPD token in PROTECTION__MASTERS (GP2->HP0 is enabled in
  # the static config above; GP3/4/5 map to HP1/2/3).
  for {set i 1} {$i < $num_dma} {incr i} {
    set gp [expr {2 + $i}]
    set_property -dict [list \
      CONFIG.PSU__USE__S_AXI_GP${gp} {1} \
      CONFIG.PSU__SAXIGP${gp}__DATA_WIDTH {128} \
    ] $zynq_ultra_ps_e_0
    set masters [get_property CONFIG.PSU__PROTECTION__MASTERS $zynq_ultra_ps_e_0]
    if {[regsub "S_AXI_HP${i}_FPD:NA;0" $masters "S_AXI_HP${i}_FPD:NA;1" masters]} {
      set_property CONFIG.PSU__PROTECTION__MASTERS $masters $zynq_ultra_ps_e_0
    }
  }

  # Create instance: axi_dma_0, and set properties.
  # Burst sizes are max (256 beats): at 250 MHz a 64-beat MM2S burst is only
  # 256 ns on the wire and the ~190 ns DDR read-latency gap between bursts
  # capped throughput at ~56% (measured on k26_mmfree370m); 256-beat bursts
  # amortize the gap to ~85%+. S2MM default (16 beats = 64 B at 32-bit width)
  # halved store bandwidth the same way.
  ...
  # DMAs 1..N-1 feed CoreTop s_axis_1..s_axis_{N-1}: MM2S only (the single
  # output stream stays on DMA0's S2MM).
  for {set i 1} {$i < $num_dma} {incr i} {
    set axi_dma_$i [ create_bd_cell -type ip -vlnv xilinx.com:ip:axi_dma:7.1 axi_dma_$i ]
    set_property -dict [list \
      CONFIG.c_addr_width {40} \
      CONFIG.c_include_sg {0} \
  ...
  # Per-DMA data SmartConnects for DMAs 1..N-1 (one MM2S master each -> its
  # own HP port, so the weight streams never share HP bandwidth).
  for {set i 1} {$i < $num_dma} {incr i} {
    set smartconnect_hp$i [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect:1.0 smartconnect_hp$i ]
    set_property -dict [list \
      CONFIG.NUM_MI {1} \
      CONFIG.NUM_SI {1} \
    ] [set smartconnect_hp$i]
  }
  ...
  # Control plane gains one AXI-Lite master per extra DMA:
  # M00 -> CoreTop.s_axi, M01 -> DMA0, M02.. -> DMA1..
  if {$num_dma > 1} {
    set_property CONFIG.NUM_MI [expr {1 + $num_dma}] $smartconnect_1
  }
  ...
  # Create instance: CoreTop_0, and set properties
  set CoreTop_0 [ create_bd_cell -type ip -vlnv user.org:user:CoreTop:1.0 CoreTop_0 ]

  # Create interface connections
  connect_bd_intf_net -intf_net CoreTop_0_m_axis [get_bd_intf_pins CoreTop_0/m_axis] [get_bd_intf_pins axi_dma_0/S_AXIS_S2MM]
  if {$num_dma == 1} {
    connect_bd_intf_net -intf_net axi_dma_0_M_AXIS_MM2S [get_bd_intf_pins CoreTop_0/s_axis] [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S]
  } else {
    # CoreTop exposes s_axis_0..s_axis_{N-1} when the stream is split; port 0
    # carries the least-significant 128 bits (lanes 0..63).
    connect_bd_intf_net -intf_net axi_dma_0_M_AXIS_MM2S [get_bd_intf_pins CoreTop_0/s_axis_0] [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S]
    for {set i 1} {$i < $num_dma} {incr i} {
      connect_bd_intf_net -intf_net axi_dma_${i}_M_AXIS_MM2S [get_bd_intf_pins CoreTop_0/s_axis_$i] [get_bd_intf_pins axi_dma_$i/M_AXIS_MM2S]
    }
  }
  ...
  connect_bd_intf_net -intf_net smartconnect_1_M00_AXI [get_bd_intf_pins CoreTop_0/s_axi] [get_bd_intf_pins smartconnect_1/M00_AXI]
  ...
  for {set i 1} {$i < $num_dma} {incr i} {
    set mi [expr {1 + $i}]
    connect_bd_intf_net -intf_net axi_dma_${i}_M_AXI_MM2S [get_bd_intf_pins axi_dma_$i/M_AXI_MM2S] [get_bd_intf_pins smartconnect_hp$i/S00_AXI]
    connect_bd_intf_net -intf_net smartconnect_hp${i}_M00_AXI [get_bd_intf_pins smartconnect_hp$i/M00_AXI] [get_bd_intf_pins zynq_ultra_ps_e_0/S_AXI_HP${i}_FPD]
    connect_bd_intf_net -intf_net smartconnect_1_M0${mi}_AXI [get_bd_intf_pins smartconnect_1/M0${mi}_AXI] [get_bd_intf_pins axi_dma_$i/S_AXI_LITE]
  }

  # Create port connections
  connect_bd_net -net CoreTop_0_irq  [get_bd_pins CoreTop_0/irq] \
  ...
  # Clocks + resets for DMAs/SmartConnects/HP ports 1..N-1 (extend the
  # existing pl_clk0 / aresetn nets). MM2S-only DMAs have no s2mm clock pin.
  for {set i 1} {$i < $num_dma} {incr i} {
    connect_bd_net [get_bd_pins zynq_ultra_ps_e_0/pl_clk0] \
      [get_bd_pins axi_dma_$i/s_axi_lite_aclk] \
      [get_bd_pins axi_dma_$i/m_axi_mm2s_aclk] \
      [get_bd_pins smartconnect_hp$i/aclk] \
      [get_bd_pins zynq_ultra_ps_e_0/saxihp${i}_fpd_aclk]
    connect_bd_net [get_bd_pins proc_sys_reset_0/interconnect_aresetn] \
      [get_bd_pins smartconnect_hp$i/aresetn]
    connect_bd_net [get_bd_pins proc_sys_reset_0/peripheral_aresetn] \
      [get_bd_pins axi_dma_$i/axi_resetn]
  }

  # Create address segments
  assign_bd_address -offset 0xA0010000 -range 0x00001000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs CoreTop_0/s_axi/reg0] -force
  ...
  # DMAs 1..N-1: AXI-Lite control windows at 0xA0020000 + (i-1)*0x10000
  # (DMA0 stays at 0xA0000000, CoreTop at 0xA0010000 — stable across N), and
  # each Data_MM2S sees DDR through its own HP port's SAXIGP{2+i}/HP{i}_*
  # segments.
  for {set i 1} {$i < $num_dma} {incr i} {
    set gp [expr {2 + $i}]
    set lite_off [format 0x%08X [expr {0xA0020000 + ($i - 1) * 0x10000}]]
    assign_bd_address -offset $lite_off -range 0x00010000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs axi_dma_$i/S_AXI_LITE/Reg] -force
    assign_bd_address -offset 0x000800000000 -range 0x000800000000 -target_address_space [get_bd_addr_spaces axi_dma_$i/Data_MM2S] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP${gp}/HP${i}_DDR_HIGH] -force
    assign_bd_address -offset 0x00000000 -range 0x80000000 -target_address_space [get_bd_addr_spaces axi_dma_$i/Data_MM2S] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP${gp}/HP${i}_DDR_LOW] -force
    assign_bd_address -offset 0xC0000000 -range 0x20000000 -target_address_space [get_bd_addr_spaces axi_dma_$i/Data_MM2S] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP${gp}/HP${i}_QSPI] -force
  }
  ...
  for {set i 1} {$i < $num_dma} {incr i} {
    set gp [expr {2 + $i}]
    exclude_bd_addr_seg -offset 0xFF000000 -range 0x01000000 -target_address_space [get_bd_addr_spaces axi_dma_$i/Data_MM2S] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP${gp}/HP${i}_LPS_OCM]
  }
